...
for ind=1:length(sweepTable)
   ...
   % npoints = bufferBeg + round(sweepTable(end)/dt) + bufferEnd;
    %npoints = bufferBeg + round(pulse_param{1}/dt) + bufferEnd;
     npoints = bufferBeg  + (2*n+1)*round(pulse_param{1}/dt) + (2*n+1)*round(2*tau/dt) + bufferEnd;
    ...
    for n_ind=1:n
        seg.addSinePulse(startPulsetime, pulse_param{1}, pulse_param{2}, pulse_param{3}, pulse_param{4});
