        ...
        function RunExperiment(obj,handles)
            ...
            for aux=1:1:handles.ExperimentalScan.vary_points(1)  %loop over first scan dimension
                ...
                if ~handles.Imaginghandles.ImagingFunctions.interfaceDataAcq.hasAborted                    
                    ...
                    %% SETUP EPR Experiment Parameters for Sequence Type 3 
                    if sequence_type == 3 % for EPR experiments
                        %setup abortable Swabian PSeq just for laser and MW
                        if ~err && ~handles.Imaginghandles.ImagingFunctions.interfaceDataAcq.hasAborted
                            disp('Send EPR sequence to Swabian');
                        ...
                        break %skip the remaining NMR exp setup and end exp
                    end
                    
                    %% SETUP NMR pulse parameters for Sequence Type 2
...
%                                     fmirror_LRpos = fmirror(1).moveRelative(117.0);
%                                     fmirror_UDpos = fmirror(2).moveRelative(-819.0);
