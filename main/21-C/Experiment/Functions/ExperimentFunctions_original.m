...
%                                 obj.interfaceSigGen2.Phase=2*aux
                              ...
                              % --------- IF USING RIGOL on channel 9
                              if size(handles.Array_PSeq{1}.Channels,2)>=9
                              if handles.Array_PSeq{1}.Channels(9).Enable %if using RIGOL IN SINGLE mode
                                  
                                  if ~isempty(handles.Array_PSeq{helper_scan,aux}.Channels(9).Frequency)
                                      
                                      if mod(aux,10)==0
                                          %DC
                                          rigol_ampl = 0;
                                          %rigol_freq = 1e-3;
                                          rigol_offset=handles.Array_PSeq{helper_scan,aux}.Channels(9).Phase;
                                          %RigolDG1022FunctionPool('setfrequency',rigol_freq);
                                          RigolDG1022FunctionPool('setamplitude',rigol_ampl);
                                          RigolDG1022FunctionPool('setoffset',rigol_offset);
                                      ...
                                      elseif mod(aux,11)==0 
                                          ...
                                          RigolDG1022FunctionPool('setoffset',rigol_offset);  
                                           ...
                                           %AC
                                          rigol_ampl = handles.Array_PSeq{helper_scan,aux}.Channels(9).Amplitude;
                                          rigol_freq = handles.Array_PSeq{helper_scan,aux}.Channels(9).Frequency;
                                          rigol_offset=0;
                                          RigolDG1022FunctionPool('setoffset',rigol_offset);
                                          RigolDG1022FunctionPool('setfrequency',rigol_freq);
                                          RigolDG1022FunctionPool('setphase',180*rand(1));
                                          RigolDG1022FunctionPool('setamplitude',rigol_ampl);
                                      ...
                                      scope{aux}.currentCH=3;
                                      TDS694cFunctionPool('setChannel',3);
                                      scope{aux}.trace=TDS694cFunctionPool('getCurve')';
                                      [scaleX,scaleY]=TDS694cFunctionPool('getScales',scope{aux}.currentCH);
                                      scope{aux}.scaleXperPoint=scaleX/50; % 10 division over X is always 500 points
                                      scope{aux}.scaleYperPoint=scaleY/(255/10);  % 8 divisions over Y is always 255 points (if DAT:Wid = 1)
                              ...
                              elseif handles.Array_PSeq{1}.Channels(10).Enable %if using RIGOL in ALTERNATING MODE
                                  ...
                                  if ~isempty(handles.Array_PSeq{helper_scan,aux}.Channels(10).Frequency)
                                      ...
                                      if mod(aux,2)==0
                                          ...
                                          rigol_ampl = 0;
                                          %rigol_freq = 1e-3;
                                          rigol_offset=handles.Array_PSeq{helper_scan,aux}.Channels(10).Phase;
                                          %RigolDG1022FunctionPool('setfrequency',rigol_freq);
                                          RigolDG1022FunctionPool('setamplitude',rigol_ampl);
                                          RigolDG1022FunctionPool('setoffset',rigol_offset);
                             ...
                             if handles.Array_PSeq{1}.Channels(5).Enable %if Electron MW enabled (Use SRS)
                              ...
                              if size(handles.Array_PSeq{1}.Channels,2)==6
                              ...
                              % --------- IF USING AWG on channel 7
                              if size(handles.Array_PSeq{1}.Channels,2)>=7
                                  if handles.Array_PSeq{1}.Channels(7).Enable
                                      
                                      if ~isempty(handles.Array_PSeq{helper_scan,aux}.Channels(7).Amplitude)
%                                           sweptParam= handles.Array_PSeq{helper_scan,aux}.Channels(7).Amplitude; %Use amplitude here as a dummy for segment number
                                          
%                                             WX1284CFunctionPool('setoutput','off');
%                                             pause(.1);

                                        mode = WX1284CFunctionPool('getmode');
                                        if strcmpi(mode(1:4),'user')
                                            WX1284CFunctionPool('selsegment',aux);
                                        elseif strcmpi(mode(1:3),'SEQ')
                                            WX1284CFunctionPool('selsequence',aux);
                                        end

%                                           pause(.1);
%                                         WX1284CFunctionPool('setoutput','on');
%                                         disp('Nothing is needed here')
                                          pause(.1);
...
%                                       end
                                  ...
                                  % If additionally using BNC
                                   if handles.Array_PSeq{1}.Channels(6).Enable
                                      
                                      if ~isempty(handles.Array_PSeq{helper_scan,aux}.Channels(6).Amplitude) && size(handles.Array_PSeq{1}.Channels,2)<11
                                          obj.interfaceBNC.Amplitude = handles.Array_PSeq{helper_scan,aux}.Channels(6).Amplitude;
                                           obj.interfaceBNC.Width = handles.Array_PSeq{helper_scan,aux}.Channels(6).Frequency; %Use frequency here as a dummy for width
%                                           Artifically set the amplitude 0
%                                           for every alternate point to
%                                           correct drift
%                                           if mod(aux,2)==0
%                                               obj.interfaceBNC.Amplitude =2;
%                                                obj.interfaceBNC.Width=50e-9;
%                                           end
                                          ...
                                          if mod(aux,2)==0
                                              obj.interfaceBNC.setStateON(4);
                                          else
                                               obj.interfaceBNC.setStateOFF(4);
                                          end
                                          pause(0.2);
                                             obj.interfaceBNC.start;
%                                            if mod(aux,2)==1
%                                           obj.interfaceBNC.start;
%                                            else
%                                                obj.interfaceBNC.stop;
%                                            end
%                                            
                                          handles.Imaginghandles.ImagingFunctions.interfacePulseGen.StartProgramming();
                                          handles.Imaginghandles.ImagingFunctions.interfacePulseGen.PBInstruction(2,handles.Imaginghandles.ImagingFunctions.interfacePulseGen.INST_CONTINUE,...
                                              0, 750,handles.Imaginghandles.ImagingFunctions.interfacePulseGen.ON);
                                          handles.Imaginghandles.ImagingFunctions.interfacePulseGen.PBInstruction(2,handles.Imaginghandles.ImagingFunctions.interfacePulseGen.INST_BRANCH,...
                                      ...
                                      elseif ~isempty(handles.Array_PSeq{helper_scan,aux}.Channels(6).Amplitude) && size(handles.Array_PSeq{1}.Channels,2)>=11
                                          obj.interfaceBNC.Amplitude = handles.Array_PSeq{helper_scan,aux}.Channels(6).Amplitude;
                                           obj.interfaceBNC.Width = handles.Array_PSeq{helper_scan,aux}.Channels(6).Frequency; %Use frequency here as a dummy for width
%                                         
                                          obj.interfaceBNC.Delay = handles.Array_PSeq{helper_scan,aux}.Channels(6).Phase; %Use phase here as a dummy for delay
...
%                                            
                                       ...
                                       % If additionally using DAQ AO1 for
                                       % voltage control
                                       if handles.Array_PSeq{1}.Channels(11).Enable
                                           handles.Imaginghandles.ImagingFunctions.interfaceDataAcq.AnalogOutVoltages(2)=handles.Array_PSeq{1}.Channels(11).Amplitude;
                                          handles.Imaginghandles.ImagingFunctions.interfaceDataAcq.WriteAnalogOutLine(2);
                                       end
                                 ...
                                 % --------- IF USING AWG on channel 8
                                 % along with PB
                              if size(handles.Array_PSeq{1}.Channels,2)>=8
                                  if handles.Array_PSeq{1}.Channels(8).Enable
                                      
                                      if ~isempty(handles.Array_PSeq{helper_scan,1}.Channels(8).Amplitude)
%                                           sweptParam= handles.Array_PSeq{helper_scan,aux}.Channels(7).Amplitude; %Use amplitude here as a dummy for segment number
                                          
%                                             WX1284CFunctionPool('setoutput','off');
%                                             pause(.1);
                                          WX1284CFunctionPool('selsegment',1);
%                                           pause(.1);
%                                         WX1284CFunctionPool('setoutput','on');
%                                         disp('Nothing is needed here')
                                          pause(.1);
...
%                                       end
                            ...
                            %PLOT! pointwise
                           cla(handles.axes_AvgData);
                           if size(handles.Array_PSeq{1}.Channels,2)>=7  &&  handles.Array_PSeq{1}.Channels(6).Enable && size(handles.Array_PSeq{1}.Channels,2)<11
                               if (handles.Array_PSeq{1}.Channels(7).Enable &&  handles.Array_PSeq{1}.Channels(6).Enable)
                               for j=1:1:number_acq
                                   hold(handles.axes_AvgData,'on');
                                   plot(x(1:2:end),temp_data{j}(1:2:end),['.-' colors(2*j-1)],'Parent',handles.axes_AvgData,'MarkerSize',15,'LineWidth',0.5);hold on;
                                   plot(x(2:2:end),temp_data{j}(2:2:end),['.-' colors(2*j)],'Parent',handles.axes_AvgData,'MarkerSize',15,'LineWidth',0.5);
                               end
                               end
                           elseif size(handles.Array_PSeq{1}.Channels,2)==10 %For RIGOL EXPERIMENTS IN ALTERNATING MODE
                               for j=1:1:number_acq
                                   hold(handles.axes_AvgData,'on');
                                   plot(x(1:2:end),temp_data{j}(1:2:end),['.-' colors(2*j-1)],'Parent',handles.axes_AvgData,'MarkerSize',15,'LineWidth',0.5);hold on;
                                   plot(x(2:2:end),temp_data{j}(2:2:end),['.-' colors(2*j)],'Parent',handles.axes_AvgData,'MarkerSize',15,'LineWidth',0.5);
                               end
                           elseif size(handles.Array_PSeq{1}.Channels,2)==9 %RIGOL EXPERIMENTS
                                   for j=1:1:number_acq
                                       hold(handles.axes_AvgData,'on');
                                       DC_array=10:10:size(x,2);
                                       Intrinsic_array=11:11:size(x,2);
                                       
                                       DC_Int_array=[DC_array Intrinsic_array];
                                       x_AC=x;
                                       x_AC(DC_Int_array)=[];
                                       x_DC=x(DC_array);
                                       x_Int=x(Intrinsic_array);
                                       y_AC=temp_data{j};
                                       y_AC(DC_Int_array)=[];
                                       y_DC=temp_data{j}(DC_array);
                                        y_Int=temp_data{j}(Intrinsic_array);
                                       
                                       plot(x_AC,y_AC,['.-' colors(3*j-2)],'Parent',handles.axes_AvgData,'MarkerSize',15,'LineWidth',0.5);hold on;
                                       plot(x_DC,y_DC,['.-' colors(3*j-1)],'Parent',handles.axes_AvgData,'MarkerSize',15,'LineWidth',0.5);
                                       plot(x_Int,y_Int,['.-' colors(3*j)],'Parent',handles.axes_AvgData,'MarkerSize',15,'LineWidth',0.5);
                                   end
                           else
...
%                     %SWITCH OFF RIGOL
%                     if size(handles.Array_PSeq{1}.Channels,2)>=9
%                         RigolDG1022FunctionPool('setoffset',0);
%                         RigolDG1022FunctionPool('setamplitude',0);
%                         RigolDG1022FunctionPool('setfrequency',1e-3);
%                         
%                         if size(handles.Array_PSeq{1}.Channels,2)==9
...
%                     end
                ...
                if Averages > 1
                    for q=1:1:number_acq
                        obj.Std_dev{q} = obj.Std_dev{q}*(1/(Averages));  % insert factor for standard error of the mean
                    end
                end
