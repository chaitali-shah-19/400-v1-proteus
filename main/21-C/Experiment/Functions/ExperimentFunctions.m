classdef ExperimentFunctions < handle
    ...
    methods
        ...
        function RunExperiment(obj,handles)
           ...
           % py.importlib.import_module('Clicker');
            warning('off','all');
...
% 
%                     %% SETUP Hyperpolarisation : MW AWT, SRS, Laser
...
%                             srs_freq=handles.Array_PSeq{helper_scan,aux}.Channels(39).FreqIQ;
%                             % Time taken to write MW chirp parameters - this takes about 10.5 seconds for Thyme
%                             MWWriteDelay = handles.Array_PSeq{helper_scan,aux}.Channels(39).Phasemod; 
%                              
% 
...
%                                     poltime=handles.Array_PSeq{helper_scan,aux}.Channels(46).FreqmodI; 
%                                     starting_pol_sign=handles.Array_PSeq{helper_scan,aux}.Channels(46).Amplitude;
%                                     LRmirror_HypPos=handles.Array_PSeq{helper_scan,aux}.Channels(46).Parameter1; 
%                                     UDmirror_HypPos=handles.Array_PSeq{helper_scan,aux}.Channels(46).Parameter2;
% 
%                                                         %% turn laser on during exp. %%
%                                     if handles.Array_PSeq{1}.Channels(10).Enable
%                                             disp('Getting laser times from NVautomizer');
%                                             LRmirror_LaserPos=handles.Array_PSeq{helper_scan,aux}.Channels(10).Parameter1;
%                                             UDmirror_LaserPos=handles.Array_PSeq{helper_scan,aux}.Channels(10).Parameter2;
%                                             laser_on_duration=handles.Array_PSeq{helper_scan,aux}.Channels(10).Frequency;
%                                             laser_on_time=handles.Array_PSeq{helper_scan,aux}.Channels(10).Phase;
%                                             laserPower=handles.Array_PSeq{helper_scan,aux}.Channels(10).Amplitude;
%                                             laserPower_pol=handles.Array_PSeq{helper_scan,aux}.Channels(10).FreqIQ;
%                                             T1_exp=handles.Array_PSeq{helper_scan,aux}.Channels(10).PhaseQ;       
%                                     end
%                 
%                                     if (poltime~=0)
%                                        % disp('Changing Laser Power for hyperpolarisation... ');
%                                        % py.Clicker.LPWAD(laserPower_pol);
%                                     end
%                                     
%                                     if (MWWriteDelay<11)
%                                         disp("Insufficient time to write microwave parameters to the AWT (recommended time is atleast 11 s)")
...
%                                         ',',num2str(sweep_freq),',',num2str(srs_freq),',',...
%                                         num2str(poltime),',',num2str(starting_pol_sign)]); 
%                                     disp(['Writing to Thyme : Will take about ', num2str(MWWriteDelay), ' seconds...'])
%                                     pause(MWWriteDelay);
%                                     Sage_write('7')
%                                 end
%                             end
%                         end
%                     end
%                     MWSetDelay = 0.1 ; %Case 7 : Time to set MW - takes about 0.05 s for Thyme to write to Tabor
%                     pause(MWSetDelay); 
%            
...
%                         pw2_current=handles.Array_PSeq{helper_scan,aux}.Channels(38).Frequency;
%                         tacq_current=handles.Array_PSeq{helper_scan,aux}.Channels(38).Phase;
%                         spacing_current=handles.Array_PSeq{helper_scan,aux}.Channels(38).PhaseQ;
%                         tref_current=handles.Array_PSeq{helper_scan,aux}.Channels(38).FreqIQ;
%                         tof_current=handles.Array_PSeq{helper_scan,aux}.Channels(38).Amplitude;
%                         expt_time_current = handles.Array_PSeq{helper_scan,aux}.Channels(38).Parameter1;
%                         pw1_current = handles.Array_PSeq{helper_scan,aux}.Channels(38).Parameter2;
%                         ExptDelay = 50; %in seconds
%                     
%                         if (T1_exp==0)
%                             ReadOutTime = expt_time_current +ExptDelay;  % in seconds
%                         else
%                             ReadOutTime = expt_time_current +ExptDelay + laser_on_time + laser_on_duration;  % in seconds
%                         end
%                       end
%                     end
% 
...
%                                 DownTime=handles.Array_PSeq{helper_scan,aux}.Channels(12).Parameter1;
%                                 UpTime=handles.Array_PSeq{helper_scan,aux}.Channels(12).Parameter2;
% 
...
%                                 obj.com.WriteVariable(ReadOutTime*1e3, 'V2', obj.com.ACSC_NONE);           
%                     
...
%                         else
%                             %%%% MAIN FIRE %%%%%%%%%%%%
%                             disp('Firing Pulse Streamer...');
%                             handles.ExperimentFunctions.swabian.stream(sequence, n_runs, finalState); %will fire Swabian   
%                             disp('Microwave and Laser triggered for hyperpolarisation...');                        
%                             pause(Wait_time);
%                             tic
%                             disp('Writing to disable Microwaves...')
%                             Sage_write('8');
%                             disp('Writing to initialise AWT...')
%                             Sage_write('1');
%                             disp('Waiting for Shuttler to move down...')
%                             disp('Writing to acquire on trigger...')
% 
% %                             % For regular experiments
% %                             Sage_write(['2,',num2str(pw1_current),',',num2str(pw2_current),',',num2str(spacing_current),',',num2str(expt_time_current),',',num2str(tacq_current),',',num2str(tref_current),',',num2str(tof_current)]); %write to Sage for initializing readout
% %For Randomised laser power experiments
%                                     Sage_write(['2,',num2str(poltime),',',num2str(pw2_current),',',num2str(spacing_current),',',num2str(expt_time_current),',',num2str(tacq_current),',',num2str(tref_current),',',num2str(tof_current)]); %write to Sage for initializing readout
%                                 disp('Writing Sage RF parameters...');     
% % for laser control                               
% %                            if (laser_on_duration~=0)
% %                                         fmirror_LRpos = fmirror(1).moveAbsolute(LRmirror_LaserPos); %Absolute Coil Pos [data]
% %                                         fmirror_UDpos = fmirror(2).moveAbsolute(UDmirror_LaserPos); %Absolute Coil Pos [data]
% %                                         mirpos=1;
...
% %                                 disp('Changing Laser Power for high-field experiments... ');
% %                                 py.Clicker.LPWAD(laserPower);
% %                            end
% 
%                             if ~handles.Imaginghandles.ImagingFunctions.interfaceDataAcq.hasAborted
%                             pause(DownTime-toc); % Time for the shuttler to move down and delay before trigger - Clicker Time
%                                  if T1_exp==1 && ~handles.Imaginghandles.ImagingFunctions.interfaceDataAcq.hasAborted
%                                     pause(laser_on_time+laser_on_duration);                             
%                                  end
%                                  
%                                  if ~handles.Imaginghandles.ImagingFunctions.interfaceDataAcq.hasAborted
%                                      disp('Writing to measure..')
%                                      Sage_write('3');
...
% 
%                             if handles.Imaginghandles.ImagingFunctions.interfaceDataAcq.hasAborted~=1
%                                 pause(ReadOutTime); % Pulse sequence and read out
%                             else
%                                    disp('You have aborted!');
%                                    obj.com.StopBuffer(1); obj.com.StopBuffer(3); obj.com.StopBuffer(7); obj.com.StopBuffer(6); obj.com.StopBuffer(9);
%                                    obj.com.KillAll();
...
%                              end
% 
%                             disp('Cleaning up and preparing for next experiment...')
%                             Sage_write('4');
%                         end
% 
%                         disp('Resetting Pulse Streamer...')
%                         handles.ExperimentFunctions.swabian.reset();  
%                         tic
% 
% %                         if (laser_on_duration~=0)
% %                             disp('Resetting laser power for hyperpolarisation...')
% %                             py.Clicker.LPWAD(laserPower_pol);
% %                         end
% 
%                         if handles.Imaginghandles.ImagingFunctions.interfaceDataAcq.hasAborted~=1
%                             pause(UpTime-toc); %Delay after read out + time for shuttler to move up 
%                             disp(['Time for shuttler to move up and gap between experiments : ',num2str(toc)])                     
%                         end   
%                         disp('Stopping shuttler program...')
%                         obj.com.StopBuffer(1); 
% 
%                         disp(['Experiment ' num2str(aux) ' complete']);
%                         disp('--------------------------------------');
% 
%                     end %End of first check    
%             end %End loop over first scan dimension end aux
end %end of RunExperiment
