...
%                                         awg_stop_freq=handles.Array_PSeq{helper_scan,aux}.Channels(39).Phase;
                                        ...
                                        % setup AWG for bandwidth and center freq
                                        % obj.tabor.awg_chirp(srs_freq-awg_start_freq,srs_freq-awg_stop_freq,awg_amp,0,1/sweep_freq);
                                        ...
                                        %try
                                            try
                                                ...
                                                rc = admin.Open();
                                                assert(rc == 0);
                                                fprintf('Opened Admin\n')
                                                
                                                slotIds = admin.GetSlotIds();
                                                numSlots = length(slotIds);
                                                assert(numSlots > 0);
                                                fprintf('Correct numSlots\n')
                                                sId = 8;
...
%                                             instId = inst.InstrId;
                                            ...
                                            assert(res.ErrCode == 0);
                                            fprintf('Clear complete\n')
                                            
                                            res = inst.SendScpi('*RST'); % reset
                                            assert(res.ErrCode == 0);
                                            fprintf('Reset command sent\n')
...
%                                             plot(dacSignal)
                                            ...
                                            res = inst.WriteBinaryData(':TRAC:DATA 0,#',dacSignal);% fliplr(dacSignal) to reverse sweep
                                            assert(res.ErrCode == 0);
                                ...
                                if length(handles.Array_PSeq{1}.Channels)>=38
                                    if handles.Array_PSeq{1}.Channels(38).Enable && ~handles.Imaginghandles.ImagingFunctions.interfaceDataAcq.hasAborted
                                        ...
                                        if sequence_type == 2  %spin lock sequence type, can uncomment others later
                                            ...
                                            [nc,Tmax]= make_pw_nmr_8(pw_current,tacq_current,gain_current,PW2_current);
                                            pause(0.1);
                                            %for next cycle
                                            make_pw_nmr_8(pw,tacq,gain,PW2);
                                            Tmax=Tmax*14*1e-3; %Tmax * number of pulses in sequence pulsed_spinlock
                                            nc=nc*14; %nc * number of aquire windows in pulsed_spinlock (?)
                                            ...
                                            loops=Tmax;
                                            %loops= 1;
                                            %below is the old calc for readlen
                                            %readLen = round2((tacq+2)*1e-6/1e-9,96)-96; %must be divisible by 96, # of samples in acq window (?)
                                            readLen = (tacq+4)*2^(-20)/4/16/(2^(-30))-96; % must be divisible by 96,
...
%                                                 end
                                                ...
                                                if handles.Array_PSeq{1}.Channels(24).Enable && ~handles.Imaginghandles.ImagingFunctions.interfaceDataAcq.hasAborted
                                                    ...
                                                    obj.com.WriteVariable(-c_position,'V3', obj.com.ACSC_NONE);
                                                    obj.com.RunBuffer(3);
                                                   
                                                    % For 2 shuttles only
                                                elseif handles.Array_PSeq{1}.Channels(8).Enable && ~handles.Imaginghandles.ImagingFunctions.interfaceDataAcq.hasAborted
...
%                                                     obj.com.RunBuffer(6);
                                ...
                                %% Main Sequence
                                        ...
                                        if handles.Imaginghandles.ImagingFunctions.interfaceDataAcq.hasAborted==1
                                            ...
                                            % turn OFF AWG and SRS (also occurs later ~line 1100)
...
%                                             end
                                            ...
                                            if handles.Array_PSeq{1}.Channels(12).Enable % For motion
                                                ...
                                                assert(res.ErrCode == 0);
                                                %Tells proteus to capture immediately on trig
%                                                 res = inst.SendScpi(':DIG:TRIG:IMM');
%                                                 assert(res.ErrCode == 0);
                                                fprintf('Waiting for trigger \n');
                                                rTrg=0;
                                                fprintf(['pulses expected =', num2str([numberOfPulses*loops]), '\n']);
                                                pause(192); %wait for shuttler to move down (92s) plus time shuttler is in ground position (100s)
...
%                                                     end
%                                                     
                                                    %Pulses is now in an array that has readLen # of rows and numberOfPulses # of columns
...
%                                                     plot_labels('Time [s]','Signal[au]');
                                                    ...
                                                    %Set up variables to store data in for QuantumStream
                                                    %pulseAmp = [];
...
%                                                 assert(rc == 0);
                                                %Is this where we put DIG INIT OFF
...
%                                     end
                            ...
                            pause(0.5);
                            fprintf('Please wait for the shuttler to finish moving! \n');
                            pause(150);
                            fprintf('End wait\n');
                            serialObj = instrfind;
                            s=size(serialObj);
