                    ...
                    if length(handles.Array_PSeq{1}.Channels)>=38
                        if handles.Array_PSeq{1}.Channels(38).Enable && ~handles.Imaginghandles.ImagingFunctions.interfaceDataAcq.hasAborted
                            ...
                            if sequence_type == 2  %spin lock sequence type, can uncomment others later
                                ...
                                [nc,Tmax]= make_pw_nmr_11(pw_current,tacq_current,gain_current,PW2_current);
                                pause(0.1);
                                %for next cycle
                                make_pw_nmr_11(pw,tacq,gain,PW2);
                                Tmax=Tmax*1*1e-3; %Tmax * number of pulses in sequence pulsed_spinlock
                                nc=nc*1; %nc * number of aquire windows in pulsed_spinlock (?)
                    ...
                    if handles.Imaginghandles.ImagingFunctions.interfaceDataAcq.hasAborted==1
                        ...
                        if handles.Array_PSeq{1}.Channels(12).Enable
                            if ~isempty(handles.Array_PSeq{helper_scan,aux}.Channels(12).Frequency)
                                ...
                                T1delay_time=handles.Array_PSeq{helper_scan,aux}.Channels(24).Frequency; %% NEW
                                ...
                                obj.com.WriteVariable(T1delay_time*1e3, 'V2', obj.com.ACSC_NONE); %writing wait time before shuttling to MotionManager
                        ...
                        if handles.Imaginghandles.ImagingFunctions.interfaceDataAcq.hasAborted==1
                            ...
                            if handles.Array_PSeq{1}.Channels(12).Enable % For motion
                                if ~handles.Imaginghandles.ImagingFunctions.interfaceDataAcq.hasAborted
                                    pause(Wait_time+3); %WAIT TIME OF 90 SECONDS + e SECONDS DELAY TO ENSURE WE DONT CUT MW OFF EARLY
                                    Sage_write(['4']);
...
%                                 disp('Wrote Sage write 4: Move down and read the data in');
                                ...
                                t.StopFcn =  @(~,~) stop(t);
                                t.StartDelay =100; %wait time in the manet
