...
%                             end
    ...
    %                             helmholtz_voltage_2 = round2((176.4 - sqrt(176.4^2 +4*29.1*(207.4-helmholtz_field_2)))/(2*(29.1)),0.01);
                                ...
                                %% SETUP Tektronix AFG 31000
                                if length(handles.Array_PSeq{1}.Channels)>=45
                                   %% when Channel 45 is enabled, turn off the output and reset the Tektronix AFG 31000.
...
%                                               obj.tabor.awg_start();
                                       ...
                                       if length(handles.Array_PSeq{1}.Channels)>=45
                                           %% when Channel 45 is enabled, turn on the burst mode for Tektronix AFG 31000 so that it can be triggered to create sin waves.
                                           if handles.Array_PSeq{1}.Channels(45).Enable && ~handles.Imaginghandles.ImagingFunctions.interfaceDataAcq.hasAborted
                                               tek_freq = handles.Array_PSeq{helper_scan,aux}.Channels(45).Frequency;
                                               tek_Vpp = handles.Array_PSeq{helper_scan,aux}.Channels(45).Amplitude;
                                               tek_DC_offset = handles.Array_PSeq{helper_scan,aux}.Channels(45).Parameter1;
                                               tek_Phase = handles.Array_PSeq{helper_scan,aux}.Channels(45).Phase;
                                               obj.tektronix_AFG_31000.burst_mode_trig_sinwave(tek_freq, tek_Vpp, tek_DC_offset, tek_Phase);
                                        ...
                                        %% SETUP SCOPE in SINGLE MODE FOR Capture
