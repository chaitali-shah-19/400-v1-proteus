...
awgWfm =        2; % 1 = square, 2 = triangle, 3 = cosine, 4 = sine
minCycles =     10;
period =        0.5E-5;
segment =       1;
...
for wfmNumber = 1:numOfWfms
    myWfm(wfmNumber, 1:(awgLength/2)) = 0.0;
end
...
for taskNumber = 1:totalTasks
    ...
    if taskNumber == 2
        ...
        inst.SendScpi(sprintf(':TASK:COMP:SEQ %d', param));
    end  
    param = 1; 
...
function wfmData = GetDigitizerData2(   inst,...
    ...
    if awgChan == 0   
        ...
        inst.SendScpi(sprintf(':DIG:TRIG:SOURCE TASK%d', awgChan));
    end
