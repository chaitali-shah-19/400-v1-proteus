%             case 6 % Program MW chirp waveform
                ...
                % Define segment 1 
                res = inst.SendScpi(strcat(':TRAC:DEF 1,',num2str(length(dacSignalMod)))); % define memory location 1 of length dacSignal
                assert(res.ErrCode == 0);
                ...
                res = inst.WriteBinaryData(':TRAC:DATA 0,#', dacSignalMod);
                assert(res.ErrCode == 0);
...
%         task table setup
        res = inst.SendScpi(':TASK:COMP:LENG 2');
        assert(res.ErrCode == 0);
        
%         TASK 1 programming:
        res = inst.SendScpi(':TASK:COMP:SEL 1');
        assert(res.ErrCode == 0);
        
        res = inst.SendScpi(':TASK:COMP:SEGM 1');
        assert(res.ErrCode == 0);
        
        res = inst.SendScpi(':TASK:COMP:TYPE SING');
        assert(res.ErrCode == 0);
        
        res = inst.SendScpi(':TASK:COMP:LOOP 0');
        assert(res.ErrCode == 0);
        
        res = inst.SendScpi(':TASK:COMP:ENAB TRG2');
        assert(res.ErrCode == 0);
        
        res = inst.SendScpi(':TASK:COMP:ABOR TRG2');
        assert(res.ErrCode == 0);
        
        res = inst.SendScpi(':TASK:COMP:JUMP IMM');
        assert(res.ErrCode == 0);
        
        res = inst.SendScpi(':TASK:COMP:DEST NEXT');
        assert(res.ErrCode == 0);
        
        res = inst.SendScpi(':TASK:COMP:NEXT1 2');
        assert(res.ErrCode == 0);
        
        res = inst.SendScpi(':TASK:COMP:KEEP OFF');
        assert(res.ErrCode == 0);
        
%         TASK 2 programming:
        res = inst.SendScpi(':TASK:COMP:SEL 2');
        assert(res.ErrCode == 0);
        
        res = inst.SendScpi(':TASK:COMP:SEGM 2');
        assert(res.ErrCode == 0);
        ...
        res = inst.SendScpi(':TASK:COMP:NEXT1 1');
        assert(res.ErrCode == 0);
