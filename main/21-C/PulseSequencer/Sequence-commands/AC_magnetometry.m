function result_seq = AC_magnetometry(seq,sample_rate,tau,length_pi_over_2,rf_freq,mod_rf,mod_mw,phase,rf_phase,ch_on)


% RF is always on during spin echo
% RF starts in the middle of first pi/2 and ends in the middle of last pi/2
switch_wait =12e-9;
Aux=0;%1*1/rf_freq;%This is offset to rf length 

T = 2*tau+4*length_pi_over_2+3*switch_wait;
RF_length=2*tau+2*switch_wait+3*length_pi_over_2-Aux;

%delay+ rise of switch
seq = ttl_pulse(seq,2,switch_wait+length_pi_over_2/2);
seq = ttl_pulse(seq,5,RF_length,0,0);
seq = ttl_pulse(seq,2,length_pi_over_2/2);
seq = wait(seq, tau);
...
seq.Channels(2).Phasemod = [seq.Channels(2).Phasemod zeros(1,int64(sample_rate*(T-length_pi_over_2))) phase*ones(1,int64(sample_rate*(length_pi_over_2)))];
...
seq.Channels(5).FreqmodI = [seq.Channels(5).FreqmodI zeros(1,int64(sample_rate*(switch_wait+length_pi_over_2/2))) mod_rf*sin(2*pi*t*rf_freq+rf_phase) zeros(1,int64(sample_rate*(length_pi_over_2/2+Aux)))]; 
