                        ...
                        if handles.Array_PSeq{1}.Channels(39).Enable && ~handles.Imaginghandles.ImagingFunctions.interfaceDataAcq.hasAborted
                            ...
                            if length(handles.Array_PSeq{1}.Channels)>=46
                                if handles.Array_PSeq{1}.Channels(46).Enable && ~handles.Imaginghandles.ImagingFunctions.interfaceDataAcq.hasAborted
                                    ...
                                    disp('Writing Microwave parameters to Thyme from Sage...');
                                    disp(['Time for writing MW: ', num2str(toc), ' seconds']);
                                    Pines_wait(2021,'6');
                                    tic
                                    Sage_write('7')
                                    Pines_wait(2022,'7');
                                    ...
                                    Sage_write(['6,',num2str(awg_center_freq),',',num2str(awg_bw_freq),',',num2str(awg_amp),...
                                        ...
                                        num2str(poltime4),',',num2str(poltime5),',',num2str(poltime6),',',num2str(starting_pol_sign)]);
                                    disp('Writing Microwave parameters to Thyme from Sage...');
                                    disp(['Time for writing MW to Thyme SG6: ', num2str(toc), ' seconds']);
                                    MWWriteDelay = 15; %Case 6 : Time taken to write MW chirp parameters - this takes about 10.5 seconds for Thyme
                                    tic
                                    Sage_write('7')
                                    disp(['Time for playing MW via Thyme SW7: ', num2str(toc), ' seconds']);
                                end
                            end
                        end
                    end
                    MWSetDelay = ; %Case 7 : Time to set MW - takes about 0.003 s for Thyme to write to Tabor
                    pause(MWSetDelay); 
                        ...
                        if handles.Imaginghandles.ImagingFunctions.interfaceDataAcq.hasAborted==1
                            ...
                            disp('Microwave and Laser triggered for hyperlorisation');
                            
                            if handles.Array_PSeq{1}.Channels(12).Enable % For motion
                                if ~handles.Imaginghandles.ImagingFunctions.interfaceDataAcq.hasAborted
                                    MWSafety =1; % 
                                    pause(Wait_time+MWSafety);                 
                                   ...
                                   % pause(15);
                                    ...
                                    fmirror_LRpos = fmirror(1).moveAbsolute(4172); %Absolute Coil Pos [data]
                                    fmirror_UDpos = fmirror(2).moveAbsolute(-4078); %Absolute Coil Pos [data]
...
%                                 t.StopFcn =  @(~,~) stop(t);
%                                 t.StartDelay =80+30-laser_on_time-laser_delay_time; %180s exp+30+T1delay_time %100=70s exp +30s wait
%                                 %t.StartDelay =10; %100
%                                 t.ExecutionMode = 'singleshot';
%                                 start(t);
%                                 wait(t);
                               pause(100); % shuttler going down

                                fmirror_LRpos = fmirror(1).moveAbsolute(3470); %Absolute Hyperpolarize Pos [data]
                                fmirror_UDpos = fmirror(2).moveAbsolute(-3610); %Absolute Hyperpolarize Pos [data]
                                fprintf('Laser mirror LR moved to hyperpolarize pos:\n %.2f.\n', fmirror(1).getPosition());
                                fprintf('Laser mirror UD moved to hyperpolarize pos:\n %.2f.\n', fmirror(2).getPosition());
%                                 py.Clicker.LPWAD(6.11);
                            end
