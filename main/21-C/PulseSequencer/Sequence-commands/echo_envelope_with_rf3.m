function result_seq = echo_envelope_with_rf3(seq,sample_rate,length_rf_pulse,wait_time_after_rf,mod,rf_freq,length_pi_over_2,s_pulse,mod_s_pulse,ch_on)


tau = (length_rf_pulse+wait_time_after_rf)/2;
T = length_rf_pulse+wait_time_after_rf+3*length_pi_over_2+s_pulse;
...
seq.Channels(5).FreqmodI = [seq.Channels(5).FreqmodI zeros(1,int64(sample_rate*length_pi_over_2)) mod*sin(2*pi*t(1:n)*rf_freq) zeros(1,int64(sample_rate*(wait_time_after_rf)/2)) zeros(1,int64(2*sample_rate*length_pi_over_2)) mod*sin(2*pi*t(n+1:end)*rf_freq) zeros(1,int64(sample_rate*(wait_time_after_rf)/2)) zeros(1,int64(sample_rate*s_pulse))];
