...
segment = 1;
 % Define segment 1 
inst.SendScpi(sprintf(':TRAC:DEF %d, %d', segment, length(dacSignal1)));   
...
segment = 2;
 % Define segment 1 
inst.SendScpi(sprintf(':TRAC:DEF %d, %d', segment, length(dacSignal2)));   
...
inst.SendScpi(':TASK:COMP:LENG 1'); 
...
inst.SendScpi(':TRIG:ACTIVE:STAT ON');
% inst.SendScpiScpi(':TRIG:SOUR:ENAB TRG1');
% inst.SendScpi(':TRIG:SLOP POS');
% inst.SendScpi(':TRIG:LEV 0.5');
% inst.SendScpi(':TRIG:COUN 0');
