 ...
 * is called, all subsequent commands (such as pb_init(), pb_set_clock(), etc.) will be
 ...
 */
SPINCORE_API int pb_inst_direct (int *pflags, int inst, int inst_data_direct,
				                 int length);
/**
...
SPINCORE_API int pb_get_data (int num_points, int *real_data,
				int *imag_data);
/**
 * Retrieve captured data from the board's memory. Use this function instead of pb_get_data()
 * if you have used the direct capture (data points are sent from the A/D directly to RAM with
 * no modification) capabilities of the board. Direct capture points are the raw 14 bit, signed
 * data from the A/D and are returned as an array of 16 bit short integers.<br>
 * You can enable direct capture by calling the function pb_set_radio_control(RAM_DIRECT)<br>
 * In direct capture mode, a USB board will acquire 4x the number of points you set with pb_set_num_points()
 * 
 * \param num_points Number of direct capture points to retrieve
 * \param data Storage space for the data points. This must contain enough room
 * \return A negative number is returned on failure, and spinerr is set to a 
 * description of the error. 0 is returned on success.
 */
/*fc below not found in lib */
/*SPINCORE_API int pb_get_data_direct (int num_points, short *data); */
  SPINCORE_API int pb_write_ascii (char *fname, int num_points, float SW,
 ...
 * description of the error. 0 is returned on success.
 */
SPINCORE_API int pb_write_ascii_verbose (char *fname, int num_points,
					   float SW, float SF, int *real_data,
					   int *imag_data);
/**
 ...
 *description of the error. 0 is returned on success.
 */
SPINCORE_API int pb_write_jcamp (char *fname, int num_points, float SW,
				   float SF, int *real_data, int *imag_data);
/**
 ...
 *description of the error. 0 is returned on success.
 */
SPINCORE_API int pb_write_felix (char *fnameout, char *title_string, int num_points, float SW, float SF, 
                int *real_data, int *imag_data);
/**
 ...
 * description of the error. 0 is returned on success.
 */
SPINCORE_API int pb_setup_fir (int num_taps, int *coef, int shift_amount,
				 int dec_amount);
/**
 ...
 *
 */
SPINCORE_API int pb_fft (int n, int *real_in, int *imag_in, double *real_out,
...
SPINCORE_API double pb_fft_find_resonance (int num_points, double SF, 
               double SW, int *real, int *imag);

//shift functions
/**
 * Shifts the data left a specified number of points and replaces
 * the right most points with a specified value
 *
 *\param num_points Number of complex data points.
 *\param points_shift Number of points to shift to the left.
 *\param val_shift Value to make the right most points.
 *\param real_in Input array of the real part of the complex data points.
 *\param imag_in Input array of the imaginary part of the complex data points.
 *\param real_out Output array of the real part of the complex data points.
 *\param imag_out Output array of the imaginary part of the complex data points.
 *
 *\return Returns the resonance frequency (in Hz).
 *
 */
/* fc below not found in lib */
/*SPINCORE_API int pb_shift_data_left (int num_points, int points_shift, int val_shift, 
			   int *real_in, int *imag_in, int *real_out, int *imag_out); */
/**
 * Shifts the data right a specified number of points and replaces
 * the left most points with a specified value
 *
 *\param num_points Number of complex data points.
 *\param points_shift Number of points to shift to the right.
 *\param val_shift Value to make the left most points.
 *\param real_in Input array of the real part of the complex data points.
 *\param imag_in Input array of the imaginary part of the complex data points.
 *\param real_out Output array of the real part of the complex data points.
 *\param imag_out Output array of the imaginary part of the complex data points.
 ...
 */
/* fc below not found in lib */
/*SPINCORE_API int pb_shift_data_right (int num_points, int points_shift, int val_shift, 
			   int *real_in, int *imag_in, int *real_out, int *imag_out); */
