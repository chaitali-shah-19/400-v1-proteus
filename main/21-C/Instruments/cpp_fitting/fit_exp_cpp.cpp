///given data as double N-dimensional vectors x and y, this function aims to fits the function P0 * (P1 + sin( P2*x + P3) )
using namespace::std;
...
{
    double sum=0.0;
    for(unsigned int ct=0; ct<x.size(); ct++)   sum+=square(y[ct] - (f_background + P[0]*exp(-P[1] * x[ct]))) ;
    return sum;
}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    my_functor.f_background=f_background[0];
    ...
    Double1D P_ini= {y_scale, 1.0/x_scale};
    Double1D step_size= {0.1*y_scale, 0.1/x_scale};
