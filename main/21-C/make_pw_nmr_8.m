function [nc,Tmax]=make_pw_nmr_8(pw,tacq,gain,PW2,tof)

    %[fid, msg] = fopen(['Z:/vnmrsys/maclib/ga_save'], 'w');
     [fid, msg] = fopen(['\\192.168.0.219\vnmrsys\maclib\ga_save'], 'w');
%      [fid, msg] = fopen(['\\192.168.0.219\vnmrsys\maclib\ga_save_FID'], 'w');
...
% pw2 = 18; %first pi/2 pulse 
 %Tmax=500;
nc_max=floor(Tmax*1e3/(neco*dtacq + PW2 + tdead)); %(Tmax*1e3) is in us! dtacq/PW2/tead is in us!
