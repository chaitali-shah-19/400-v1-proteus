...
x_axis=[100:-4:0];
scaled_pump_enh =  [34.8564   33.0966   33.0415   32.7595   33.6538   32.3849   32.9814   32.6677   30.9990   30.7821   29.8718   32.1558   29.8006   28.0244   27.7827 ...
   28.7774   27.7314   28.9249   27.2025   24.3758   20.6477   18.8840   16.5797   13.6047    8.0238];
[f2,xvec2,fitted_contrast2]= fit_exp2(25:-1:1,scaled_pump_enh,25:-1:1);
...
explimits{1}='2017-07-12_15.51.44';
explimits{2}='2017-07-12_15.51.44';
...
%     [enhancementFactor(k),fwhm3]=process_enhancement(spec2,spec3,0,1);
      ...
      [enhancementFactor(k),fwhm3,spec20{j},spec30{j}]=process_enhancement(spec2,spec3,15);
...
%     load mean_enhancement
    x_axis=[1000:-25:25];
 [f2,xvec2,fitted_contrast2]= fit_exp2(x_axis,mean_enhancementFactor,x_axis);
 start_fig(12,0.75*[2.25 2]);
 p1=plot_preliminaries(x_axis*5.69/100,mean_enhancementFactor,2);
 set(p1,'markersize',7,'linewidth',0.5);
 p1=plot_preliminaries(xvec2*5.69/100,fitted_contrast2,2,'nomarker');
    plot_labels('Laser Power [W]','Enhancement over 7T');
  set(gca,'tickdir','in');
 box on;
